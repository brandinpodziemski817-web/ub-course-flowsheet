...
一个可交互的生物医学工程八学期课程流程图，用于展示课程之间的先修、共修和后续依赖关系。

> 本项目是对课程流程图交互方式的独立复现，仅用于学习、展示和产品原型验证，不是纽约州立大学布法罗分校（University at Buffalo）的官方选课工具。课程要求、课程替代和毕业审核请以学校最新目录及学业导师意见为准。
...
课程数据位于 `dist/index.html` 底部 `<script>` 中的 `courses` 数组。每门课程使用以下结构：

```javascript
C('课程代码', '课程名称', 学分, 学期, ['先修课程'], ['共修课程'], '课程说明')
```
